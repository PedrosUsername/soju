...
def getGeneralsAvoidRepetition(generals= None) :
    if (
        generals is None
    ) :
        return None
        ...
        return generals.get("avoidrepetition")

def getGeneralsRandomTimestamps(generals= None) :
    if (
        generals is None
    ) :
        return None
        ...
        return generals.get("randomtimestamps")
...
def prepare_boomer_gen_generals(generals= None) :
    ...
    if type(generals) is dict :
        ...
        healthy_generals["avoidrepetition"] = getGeneralsAvoidRepetition(generals)
...
def buildBoomer(obj, image_file_dirs= {}, audio_file_dirs= {}, video_file_dirs= {}, generator= None):
    ...
    general = generator.get("generals") if generator.get("generals") else {}
    default = generator.get("defaults") if generator.get("defaults") else {}
    ...
    if default.get("image") is not None :
        ...
        for param in default.get("image") :
            ...
            if general.get("avoidrepetition") and image_file_dirs.get(param_dir):
                image_file_dirs.get(param_dir).remove(new_b_param["file"])
    ...
    if default.get("audio") is not None :
        ...
        for param in default.get("audio") :
            ...
            if general.get("avoidrepetition") and audio_file_dirs.get(param_dir) :
                audio_file_dirs.get(param_dir).remove(new_b_param["file"])            
    ...
    if default.get("video") is not None :
        ...
        for param in default.get("video") :
            ...
            if general.get("avoidrepetition") and video_file_dirs.get(param_dir) :
                video_file_dirs.get(param_dir).remove(new_b_param["file"])
